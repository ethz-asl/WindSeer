...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', '--binnning_method', default='binning', help='Method used to bin the flight data into the grid')
    ...
    parser.add_argument('-p', '--predict', action='store_true', help='Predict the full field in case the binning method supports it')
    ...
    t_start = time.time()
    wind, variance, measurements_mask, prediction = bin_log_data(wind_data, grid_dimensions, args.binnning_method, True, args.predict)
    print(' done [{:.2f} s]'.format(time.time() - t_start))
    ...
    utils.plot_measurement(['ux', 'uy', 'uz'], wind, measurements_mask,
                           terrain = terrain, variance = variance, prediction = prediction)
