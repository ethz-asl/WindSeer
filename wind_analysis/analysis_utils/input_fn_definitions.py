...
def get_boundary_layer(n_cells, start_cell, roughness):
    profile = torch.zeros(n_cells)

    num_cells = n_cells - start_cell
    profile[start_cell:] = torch.log((torch.arange(num_cells) + roughness) / roughness)
...
def bp_corners_1(params, terrain, interpolator, config):
    ...
    for i in [0, -1]:
        for j in [0, -1]:
            ...
            profile = get_boundary_layer(n_z, start_idx, roughness).to(device) * params[0]
...
def bp_corners_4(params, terrain, interpolator, config):
    ...
    for i in [0, -1]:
        for j in [0, -1]:
            ...
            profile = get_boundary_layer(n_z, start_idx, roughness).to(device) * params[corner_idx]
