...
print('Input data: ', scale_keys[args.index])

terrain = torch.from_numpy(h5_file[scale_keys[args.index]]['terrain'][...])
input_meas = torch.zeros(tuple([3]) + tuple(terrain.shape))
input_mask = torch.zeros(tuple(terrain.shape))
ds_input = h5_file[scale_keys[args.index]][args.input_mast]
...
if input_mask.sum() == 0:
    raise ValueError('All measurements lie inside the terrain or outside of the prediction domain')
...
for mast in ['M0', 'M1', 'M2', 'M3', 'M4', 'M5', 'M6', 'M7', 'M8', 'M9']:
    ds_mast = h5_file[scale_keys[args.index]][mast]
