...
def get_terrain(terrain_geotiff, x, y, z=None, resolution=(64,64,64), build_block=True, plot=False, distance_field=False,
                horizontal_overflow = 0, return_is_wind = False):
    """
    ...
    Inputs:
        ...
        build_block: flag to output full_block
        plot: Plot the extracted terrain
        distance_field: Return the full_block as a distance field instead of binary map
        horizontal_overflow: Extract a larger portion of the map, build the distance field and then extract the actual extent.
                             It is forced to be an integer and the unit is in number of cells.
        return_is_wind: In case of a binary full_block the cells will be true where there is wind, else terrain cells are true
    ...
    Outputs:
          ...
          full_block:        terrain array (size is resolution input argument) dimension order is [z, y, x].
    ...
    idx_to_keep_x = np.logical_and(x_img < extended_bounds_x[1], x_img > extended_bounds_x[0])
    idx_to_keep_y = np.logical_and(y_img < extended_bounds_y[1], y_img > extended_bounds_y[0])

    interp_spline = RectBivariateSpline(y_img[idx_to_keep_y],
                                        x_img[idx_to_keep_x],
                                        z_img[idx_to_keep_y][:, idx_to_keep_x])
    ...
    if build_block:
        ...
        if distance_field:
            ...
            full_block = np.zeros((resolution[0] + offset_cells_z, z_out.shape[0], z_out.shape[1]), dtype='bool')
        else:
            offset_cells_z = 0
        ...
        if distance_field:
            ...
            full_block = full_block[offset_cells_z:, horizontal_overflow:-horizontal_overflow, horizontal_overflow:-horizontal_overflow]
