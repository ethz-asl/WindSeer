...
class TestDictutils(unittest.TestCase):
    ...
    def test_dict_update(self):
        dict_in = {'a': 1, 'b': {'changed': 2}, 'changed': 3, 'c': {'unchanged': 3}}
        dict_update = {'b': {'changed': 4}, 'new_field': 7, 'newgroup': {'val': 0}}
        dict_out_gt = {'a': 1, 'b': {'changed': 4}, 'changed': 3, 'c': {'unchanged': 3}, 'new_field': 7, 'newgroup': {'val': 0}}
    ...
    def test_tensors_to_dtype(self):
        dict_in = {'1': torch.zeros(1,1, dtype=torch.float), '2': {'1': torch.zeros(1,1,dtype=torch.float)}}
