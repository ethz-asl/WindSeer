def compute_terrain_factor(samples, terrain):
    ...
    if (len(samples.shape) != 5) or (len(terrain.shape) != 5):
        raise ValueError('compute_terrain_factor: only defined for 5D data. Unsqueeze single samples!')

    if (samples.shape[0] != terrain.shape[0]):
        raise ValueError(
            'compute_terrain_factor: batchsize of samples({}) and terrain({}) are different.'.format(samples.shape[0],
                                                                                                     terrain.shape[0]))

    if (samples.shape[-3:] != terrain.shape[-3:]):
        raise ValueError('compute_terrain_factor: samples and terrain must have same domain shape and resolution.')

    # get amount of data that is not in the terrain for each sample
    fluid_elements_in_samples = terrain.sign_().sum(-1).sum(-1).sum(-1) * samples.shape[1]
